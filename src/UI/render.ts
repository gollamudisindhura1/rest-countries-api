...
export function renderCountries(countries: Country[]) {
  $.grid.innerHTML = "";
  countries.forEach(c => {
...
function showDetail(c: Country) {
  const flagImg = document.querySelector(".flag-detail-img") as HTMLImageElement;
...
setText(".detail-name", displayName);
  ...
  setText(".detail-currencies", 
    c.currencies 
      ? Object.values(c.currencies).map((x: any) => x.name).join(", ") 
      : "N/A"
  );
  setText(".detail-languages", 
    c.languages 
...
document.querySelectorAll(".list-item").forEach(item => {
  item.addEventListener("click", e => {
    ...
    if (btn) btn.textContent = region || "Filter by Region";
  });
});